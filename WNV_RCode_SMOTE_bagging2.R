...
for (i in 1:repeats) {
    ...
    for (j in 1:K) {
        ...
        output = vector()
        for (p in 1:length(ntree.list)){
            mod = randomForest(fit.formula, train.data[block!=j,], 
                               mtry=NumVariable, ntree=ntree.list[p])
            ###############################
            ...
            temp = cbind(ntree=ntree.list[p],cutoff=table1[which.max(table1[,2]),1], auc=table1[which.max(table1[,2]),2])
            output = rbind(output, temp)
        }
        #################################################
        ## Find the optimal ntree and probability cutoff
        #################################################
        par.ntree  = output[which.max(output[,3]),1]
        par.cutoff = output[which.max(output[,3]),2]
        
        ###########################################################
        ## Use the optimal ntree to construct new model  
        ###########################################################
        mod = randomForest(fit.formula, train.data, 
                           mtry=NumVariable, ntree=par.ntree)
...
## write output
write.csv(summary.table, "D:/Pinsker/Kaggle/kaggle/predict_smote_bagging2.csv")
