...
for (i in 1:repeats) {
    ...
    for (j in 1:K) {
        #======[Put your model here]==============================================================
        fit.formula = as.formula( ~ Month + Species + Tavg.ordinal + RH.ma4 + DewPoint.ma1 
                                  + LowWind.byMean + daytime + log.HotSpot + NumMosquitos)
        x = model.matrix(fit.formula, train.data[block!=j,])[,-1]
        y = train.data[block!=j,c("WnvPresent")]
        x.test = model.matrix(fit.formula, train.data[block==j,])[,-1]
        y.test = train.data[block==j,c("WnvPresent")]

        mod.lasso = cv.glmnet(x, y, alpha=1, family=c("binomial"))
        bestlam = mod.lasso$lambda.min
        
        ###############################
        ## Optimize probability cutoff
        ###############################
        mod.lasso.pred.train = predict(mod.lasso, newx=x.test, s=bestlam, type=c("response"))
        table1 = f.cutoff.optimizer(pred=mod.lasso.pred.train, y=y.test)
        cutoff = table1[which.max(table1[,2]),1]
        
        ###########################################################
        ## Use the optimal cutoff to predict the test.data dataset 
        ###########################################################
        x.test = model.matrix(fit.formula, test.data)[,-1]
        test.pred = predict(mod.lasso, newx=x.test, s=bestlam, type=c("response"))
...
## write output
#write.csv(summary.table, "predict_lasso.csv")

apply(summary.table,2,mean)
#Accuracy Sensitivity Specificity       Kappa 
#0.7728689   0.8237226   0.7313988   0.5475991 
